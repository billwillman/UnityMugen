...
public static class CustomSettings  {
	...
	{        
        ...
        _DT(typeof(System.Func<LuaTable, string, bool>)),

		_DT(typeof(System.Action<float, bool, GameObject>)),
		_DT(typeof(Action<bool>)),
		_DT(typeof(Action<AsyncOperation>)),
		_DT(typeof(Action<float, bool, Texture>)),
		_DT(typeof(Action<float, bool, Material>)),
		_DT(typeof(Action<float, bool, AudioClip>)),
		_DT(typeof(Action<float, bool, TextAsset>)),
		_DT(typeof(Action<float, bool, AnimationClip>)),
		_DT(typeof(Action<float, bool, RuntimeAnimatorController>)),
		_DT(typeof(Action<float, bool, Shader>)),
		_DT(typeof(Action<float, bool, Font>)),
	...
	{                
		...
		*/
		...
		_GT(typeof(ResourceCacheType)),
		_GT(typeof(ResourceMgr)),
		_GT(typeof(PlayerAttribe)),
		_GT(typeof(PlayerDisplay)),
        _GT(typeof(Cns_Type)),
	...
	private static void ClearLuaFileDir(string rootDir)
	{
		if (string.IsNullOrEmpty(rootDir) || !Directory.Exists(rootDir))
			return;
		try
		{
			string[] luafiles = Directory.GetFiles(rootDir, "*.lua.bytes", SearchOption.AllDirectories);
			if (luafiles != null)
			{
				for (int i = 0; i < luafiles.Length; ++i)
				{
					string fileName = luafiles[i];
			...
			Debug.Log("Lua Files Delete Sucess!");
		}
		catch 
		{
			Debug.LogError("Lua Files Delete Error!");
		}
	} 
}
